...
const CasinoWcProviders = () => {
  ...
  React.useEffect(() => {
    ...
    const provArr = Object.keys(gameList).map((provId) => {
      const title = getTitleWB(Number(provId));
      return {
        _id: provId,
    ...
    if (location.pathname === '/' && !category && provArr[0]?._id) {
      navigate(`/wc-providers/wc-providers?category=${provArr[0]._id}`, { replace: true });
      return;
    }
  ...
  "11": "https://static.live88.io/game/ld-client/latest/assets/logo-with-text-DMGCBD_O.svg",
};
              ...
              {providers.map((prov: any, key: number) => (
                <li className="nav-item" key={key}>
                  <CustomLink
                    ...
                    to={`/wc-providers/${queryp.type}?category=${prov._id}`}
                  >
...
export default CasinoWcProviders