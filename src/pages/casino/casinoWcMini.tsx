    ...
    const launchGame = async () => {
      ...
      try {
        const response = await fetch(
          `${process.env.REACT_APP_API_BASEURL_V1}launchZapcoreGame`,
          {
            method: "POST",
            headers: {
    ...
    <>
      <div className="container-fluid container-fluid-5 cas-in-list">
        <div className="row row5">
          {/* Provider Sidebar */}
          <div
            className="col-xl-2 d-none d-xl-flex"
            style={{ marginTop: "5px" }}
          >
            <ul className="nav nav-pills casino-sub-tab">
              {providers.map((prov: any, key: number) => (
                <li className="nav-item" key={key}>
                  <CustomLink
                    className={`nav-link ${
                      prov._id == activeProvider ? "active" : ""
                    }`}
                    to={`/wc-mini/${queryp.type}?category=${prov._id}`}
                  >
                    <span>
                      {prov.title_first} {prov.title_second}
                    </span>
                  </CustomLink>
                </li>
              ))}
